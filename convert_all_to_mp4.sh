...
main (){
    ...
    while getopts ": i: o: d:" opt; do
        case ${opt} in
            ...
            o) echo "Output format set to: $OPTARG"
                format_out=$OPTARG
            ;;
            d) echo "Directory set to: $OPTARG"
    ...
    count_mkv_files=$(wc -w <<< "$all_file_in")
    if [ "$count_mkv_files" -eq "0" ]; then
        echo "No .${format_in} files found in the current directory."
        exit 0
    fi
    echo "Found $count_mkv_files .${format_in} files. Starting conversion to .${format_out}..."
    ...
    for file_in in $all_file_in; do
        ...
        if (ffmpeg -i "$file_in" "$file_out" >> conversion.log 2>&1); then
